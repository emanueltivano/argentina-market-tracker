...
export function isDemoSymbolSupported(symbol: string): boolean {
  return symbol.toUpperCase() in DEMO_SYMBOL_PROFILES
}

export function getDemoSymbolDescription(symbol: string): string | null {
  return DEMO_SYMBOL_PROFILES[symbol.toUpperCase()]?.description ?? null
}
